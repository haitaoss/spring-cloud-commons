...
public class BaseApp {
	...
	public Object call(@PathVariable("serviceName") String serviceName) {
		return restTemplate.getForEntity(String.format("http://%s/name", serviceName), String.class).getBody();
	}

	@RequestMapping("/2/{serviceName}")
	public Object call2(@PathVariable("serviceName") String serviceName) {
		return webClientBuilder.build().get().uri(String.format("http://%s/name", serviceName)).header("x", "x")
